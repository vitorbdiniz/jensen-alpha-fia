...
def get_prices(persist = True, verbose = False):
    ...
    results = pd.DataFrame(columns=["codigo_cvm", "ticker_id","codigo_negociacao","date","High","Low","Open","Close","Volume","Adj Close"])
    for i in range(len(tickers["codigo_negociacao"])):
        t = tickers["codigo_negociacao"].loc[i].replace("$", "") + ".SA"
        if verbose:
            print(str(i+1) + ". Buscando preços de " + str(t) + " -- restam: " + str(len(tickers["codigo_negociacao"])-i))
        try:
            prices = web.get_data_yahoo(t, "2010-01-01", date.today())
            ...
            prices["codigo_negociacao"] = [tickers["codigo_negociacao"].loc[i] for x in range(len(prices["High"]))]
